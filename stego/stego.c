...
void hide_char(unsigned char c, uint8_t k, uint32_t* pixels, uint64_t* pixel)
{
...
void get_char(const uint32_t* pixels, uint8_t k, unsigned char* c, uint64_t* pixel)
{
    uint8_t mask = BITS8;
    uint8_t shift = CHAR_SIZE;
    *c &= ~(mask);
...
uint8_t padding_jump(uint64_t prev_pixel, uint8_t k, uint64_t padding_left)
{
...
int hide_text(const unsigned char* text, uint32_t x, uint32_t y, uint8_t k, uint32_t* pixels)
{
    int8_t i;
    uint8_t pixels_per_char;
    uint16_t text_size_bits, jump;
    uint64_t padding_left;
    uint8_t to_hide = 0;
    uint16_t text_size_chars = strlen(text);
    uint64_t pixel = 0,  usable_pixels = x * y - 3;

    if (k < 1 || pixels == NULL || text == NULL || text_size_chars > MAX_TEXT_SIZE)
        return 0;

    text_size_bits = (text_size_chars + 1) * CHAR_SIZE;
...
{
    int8_t i;
    uint8_t to_get, text_size_chars, pixels_per_char;
    uint16_t jump;
    uint64_t padding_left;
    uint8_t k = 0;
    uint64_t pixel = 0, usable_pixels = x * y - 3;
